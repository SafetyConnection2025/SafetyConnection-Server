package org.example.safetyconnection.member.controller;
...
import org.example.safetyconnection.companion.service.facade.CompanionService;
...
import org.example.safetyconnection.member.service.facade.MemberService;
...
public class MemberController {
  ...
  public ResponseEntity<List<CompanionResDTO>> findAllCompanions(
  ...
  public ResponseEntity<CompanionResDTO> addCompanion(HttpServletRequest httpServletRequest,
  ...
  public ResponseEntity<MemberLocationResDTO> addLocation(HttpServletRequest httpServletRequest,
    @RequestBody MemberLocationReqDTO memberLocationReqDTO) {
    String accessToken = httpServletRequest.getHeader("access");
    Long userId = jwtTokenProvider.getUserIdFromToken(accessToken);
    log.info("L_POST 요청 수신: userId = {}", userId);

    MemberLocationResDTO memberLocationResDTO = memberService.setLocation(userId,
      memberLocationReqDTO);
  ...
  public ResponseEntity<FCMTokenResDTO> updateToken(@PathVariable("userId") Long userId,
  ...
  public ResponseEntity<String> deleteCompanion(HttpServletRequest httpServletRequest,
    @PathVariable Long compId) {
    String accessToken = httpServletRequest.getHeader("access");
    Long userId = jwtTokenProvider.getUserIdFromToken(accessToken);
  ...
  public ResponseEntity<DetectedObjectResponseDTO> detectObject(
    @RequestBody DetectedObjectRequestDTO detectedObjectRequestDTO) {
    DetectedObjectResponseDTO detectedObjectResponseDTO = yolov8DetectionSerivce.detect(
      detectedObjectRequestDTO);
